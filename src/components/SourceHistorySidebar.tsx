...
export const SourceHistorySidebar: React.FC<SourceHistorySidebarProps> = ({ onSelectURL, onSelectFile, open, onClose }) => {
  const { sources, removeSource, clear } = useSourceHistory();

  return (
    <div className={`fixed inset-0 z-40 ${open ? "" : "pointer-events-none opacity-0"} flex`}>
      <div className="fixed inset-0 bg-black/30" onClick={onClose} aria-label="Close source sidebar" />
      <Sidebar className="relative w-[320px] bg-white/90 shadow-xl border-r border-slate-100 h-full animate-slide-in-left">
        <SidebarContent>
          <SidebarGroup>
            <SidebarGroupLabel>
              Source History
              <button className="ml-3 text-xs px-2 py-1 rounded bg-slate-100 text-slate-700 hover:bg-slate-200" onClick={clear}>
                Clear All
              </button>
            </SidebarGroupLabel>
            <SidebarGroupContent>
